    ...
    public partial class Form1 : Form
    {
        ...
        private void акварельToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (full_name_of_image != "\0")
            {
                ...
                pixel = Filter.matrix_filtration(image.Width, image.Height, pixel, Filter.N3, Filter.aqua);
                pixel = Filter.matrix_filtration(image.Width, image.Height, pixel, Filter.N3, Filter.aqua_after);
                FromPixelToBitmap();
                FromBitmapToScreen();
            }
        }
        ...
        private void инверсияToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (Form1.full_name_of_image != "\0")
            {
                ...
                for (int i = 0; i < Form1.image.Height; i++)
                    ...
                    {
                        p = Filter.Invers(Form1.pixel[i, j]);
        ...
        private void тиснениеToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (full_name_of_image != "\0")
            {
                ...
                pixel = Filter.matrix_filtration(image.Width, image.Height, pixel, Filter.N3, Filter.tisnenie);
               // pixel = Filter.matrix_filtration(image.Width, image.Height, pixel, Filter.N3, Filter.aqua_after);
                FromPixelToBitmap();
                FromBitmapToScreen();
            }
